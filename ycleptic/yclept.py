...
def _make_doc(L,topname,toptext,fp,docname='',doctext='',docexample={},rootdir='',footer_style='paragraph'):
    ...
    if len(sd)>0:
        ...
        for s in sd:
            ...
            doc=s.get('docs',{})
            with open(f'{name}.rst','w') as f:
                _make_doc(s['directives'],name,s['text'],f,docname=doc.get('title',''),doctext=doc.get('text',''),docexample=doc.get('example',{}),rootdir=rootdir)
        os.chdir('..')
...
*This file was automatically generated by **{app_name}** v{version} on {today}.*
""".strip()
...
def cli():
    ...
    command_parsers['make-doc'].add_argument('--footer-style',type=str,default='paragraph',choices=['paragraph','comment','rubric'],
                                             help='footer style for the generated documentation; one of "paragraph", "comment", or "rubric"')
